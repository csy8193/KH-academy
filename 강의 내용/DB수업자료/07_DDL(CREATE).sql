...
/* 자료형
    ...
    CHAR(크기) : 고정길이 문자형 (2000BYTE) 
        ...
        -> 고정된 길이의 문자열 저장 시 좋음(EX. 주민등록번호)
        
    VARCHAR2(크기) : 가변길이 문자형 (4000 BYTE)
        -> ex) VARCHAR2(10) 컬럼에 'ABC' 3BYTE 문자열만 저장하면 나머지 7BYTE를 반환함.
        -> 고정되지 않은 문자열 저장 시 좋음
        + CHAR, VARCHAR2는 문자의 바이트 크기를 기준으로 저장하기 때문에 작성되는 문자 수와는 관계 없음.
        -> 영어, 숫자, 기본 특수문자 == 1BYTE
        나머지 문자(한글 등) == 3BYTE
    ...
    NVARCHAR2(길이) : 가변길이 문자 개수 자료형
    길이는 문자의 개수, 최대 저장 용량은 4000BYTE
    EX) NVARCHAR2(2000) : 2000글자 저장 가능한 컬럼
    -> 한글 2000글자 == 6000BYTE -> X
...
-- VARCHAR2(30) : 가변길이 문자열 30바이트
-- DEFAULT : 컬럼 기본 값
    --> INSERT, UPDATE 시 해당 컬럼에 아무 값도 입력되지 않거나, DEFAULT로 작성된 경우
    --  DEFAULT 오른쪽에 작성된 값을 삽입
...
INSERT INTO MEMBER VALUES ('MEM01', '123ABC', '홍길동', '2021-09-24');
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO MEMBER VALUES ('MEM02', 'QWER1234', '김영희', SYSDATE);
SELECT * FROM MEMBER;

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO MEMBER VALUES ('MEM03', 'ZZ9876', '박철수', DEFAULT);
SELECT * FROM MEMBER;

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO MEMBER (MEMBER_ID, MEMBER_PWD, MEMBER_NAME) VALUES ('MEM04', '1234ASDF', '이지연');
SELECT * FROM MEMBER;
...
CREATE TABLE USER_USED_NN(
    -- USER_NO NUMBER NOT NULL,  -- 컬럼 레벨 제약 조건 (제약조건명 미지정 -> SYS_XXXXX) 
    USER_NO NUMBER CONSTRAINT USER_NO_NN NOT NULL, -- 제약 조건명 지정
    USER_ID VARCHAR2(20) ,
...
CREATE TABLE USER_USED_UK(
    USER_NO NUMBER,
    -- USER_ID VARCHAR2(20) UNIQUE, -- 컬럼 레벨(제약 조건명 X) 
    ...
    --UNIQUE(USER_ID) -- 테이블 레벨(제약 조건명 X)
...
CREATE TABLE USER_USED_PK(
    --USER_NO NUMBER PRIMARY KEY , -- 컬럼레벨 (제약 조건명 미지정)
    --USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼레벨 (제약 조건명 지정)
    USER_NO NUMBER,
    ...
    PHONE VARCHAR2(30),
    EMAIL VARCHAR2(50),
    --PRIMARY KEY(USER_NO) -- 테이블 레벨 (제약 조건명 미지정)
    CONSTRAINT USER_NO_PK PRIMARY KEY(USER_NO) -- 테이블 레벨 (제약 조건명 지정)
);
...
-- USER_GRADE 테이블의 PK 인 GRADE_CODE와 FK 제약조건 설정하기 (== 관계 맺기)
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  --GRADE_CODE NUMBER REFERENCES USER_GRADE(GRADE_CODE) -- 컬럼 레벨(제약 조건명 미지정)
  --GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE(GRADE_CODE)  -- 컬럼 레벨(제약 조건명 지정)
  GRADE_CODE NUMBER,
  --FOREIGN KEY (GRADE_CODE) REFERENCES USER_GRADE -- 테이블 레벨(제약 조건명 미지정, 참조 컬럼 생략)
...
DELETE FROM USER_GRADE
WHERE GRADE_CODE = 10;
--> 부모 테이블의 GRADE_CODE = 10을 삭제하려고 했으나
--  자식 테이블이 10이라는 값을 기록하고 있어서 삭제 불가
...
CREATE TABLE USER_USED_FK2(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER REFERENCES USER_GRADE2 ON DELETE SET NULL
...
CREATE TABLE USER_USED_FK3(
  ...
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER,
  FOREIGN KEY (GRADE_CODE) REFERENCES USER_GRADE3 ON DELETE CASCADE -- 테이블 레벨
...
CREATE TABLE USER_USED_CHECK(
  ...
  USER_PWD VARCHAR2(30) NOT NULL,
  USER_NAME VARCHAR2(30),
  --GENDER VARCHAR2(10) CHECK (GENDER IN ('남','여') ), -- 컬럼 레벨(제약 조건명 미지정)
...
CREATE TABLE CHECK_TEST (
    ...
    INPUT_NUMBER NUMBER CHECK(INPUT_NUMBER > 0) -- INPUT_NUMBER 컬럼에는 0보다 큰 수만 삽입 가능
);

INSERT INTO CHECK_TEST VALUES(1, 10);
INSERT INTO CHECK_TEST VALUES(2, -5);


/* 
NOT NULL : 컬럼 값 NULL 불가
UNIQUE : 같은 컬럼 내 중복 값 불가
PRIMARY KEY : 식별자 (모든 행 구분 역할) , NOT NULL + UNIQUE
FOREIGN KEY : 부모 테이블 컬럼이 제공하는 값만 자식 테이블 컬럼에서 사용 가능 + NULL도 가능
...
* FK 제약조건의 삭제 옵션
ON DELETE SET NULL : 부모 삭제시 자식 NULL로 변경 
ON DELETE CASCADE : 부모 삭제시 자식 행 삭제
...
WHERE 1 = 0; -- 무조건 FALSE인 상황을 만들어 데이터가 조회되지 않게 함.
