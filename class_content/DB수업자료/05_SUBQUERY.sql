...
--> 기존 서브쿼리 SQL 해석 순서 = 1) 서브쿼리, 2) 메인쿼리
--> 상관 서브쿼리 SQL 해석 순서 = 1) 메인쿼리, 2) 서브쿼리

-- 상관쿼리는 먼저 메인쿼리 한 행을 조회하고
-- 해당 행이 서브쿼리의 조건을 충족하는지 확인하여 SELECT를 진행함

-- EXISTS : 서브쿼리 결과가 1행 이상 존재하는가?
-- 사수가 있는 직원의 사번, 이름, 부서코드, 사수사번 조회
--> 이태림 제외
...
--> SELECT문은 테이블 내에서 조건에 맞는 행을 한 번에 조회 하는 것이 아닌
-- 테이블의 한 행씩 접근하여 조건을 하나하나 따져 일치하면 조회 결과 집합(RESULT SET)에
-- 해당 행을 추가하여 모든 행을 따져 본 후 결과를 조회
...
WHERE HIRE_DATE IN (SELECT MIN(HIRE_DATE)
                    ...
                    WHERE ENT_YN = 'N'
                    GROUP BY DEPT_CODE);
--> 결과는 맞게 나왔으나, 조건이 불명확함
--> 서브쿼리 결과가 단순히 입사일만을 조회했기 때문에
--  만약 같은 입사일인 사람이 있으면 모두 조회되었을 것이다.
...
SELECT EMP_NAME FROM EMPLOYEE WHERE JOB_CODE = 'J1';

SELECT EMP_NAME, DEPT_CODE, (SELECT EMP_NAME FROM EMPLOYEE WHERE JOB_CODE = 'J1') 대표
FROM EMPLOYEE;
...
SELECT EMP_ID, EMP_NAME, NVL(MANAGER_ID, '없음'),
    NVL((SELECT EMP_NAME FROM EMPLOYEE B
     WHERE A.MANAGER_ID = B.EMP_ID), '없음') "관리자명"
FROM EMPLOYEE A;
...
-- VIEW : 가상의 테이블(진짜 테이블이 아님) --> EX) SELECT 결과(RESULT SET)
