...
-- 21행이 조회되는 이유 ==> 이오리, 하동운이 DEPT_CODE가 NULL이기 때문에
-- 조인하는 DEPARTMENT의 DEPT_ID컬럼에 연결되는 값이 없어서 제외됨.

-- 오라클
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT -- 오라클 방식은 사용할 테이블을 모두 FROM절에 작성
WHERE DEPT_CODE = DEPT_ID; -- 연결고리가 되는 두 컬럼의 값이 같은 경우에 연결
...
--> 이오리, 하동운 행은 DEPARTMENT 테이블과 연결되는 컬럼이 없으므로
-- DEPARTMENT 관련 컬럼 조회 시 모두 NULL 값을 가지게 된다.
...
--> 실제로 사용되는 경우는 적지만
--> 조인이 잘못 되었을 경우 조회 결과가 크로스 조인의 형태로 조회된다
---> 조인해쓴ㄴ데 크로스 조인 결과가 나옴 -> 조인을 잘못함
...
-- EMPLOYEE 테이블에서
-- 자신의 급여 등급에 맞는 급여를 받는 직원의
-- 이름, 급여등급, 급여, 최소급여, 최대급여 조회
SELECT EMP_NAME, E.SAL_LEVEL, SALARY, S.MIN_SAL, S.MAX_SAL
FROM EMPLOYEE E
JOIN SAL_GRADE S ON (E.SALARY BETWEEN MIN_SAL AND MAX_SAL);

-- 오라클 사용 가능 비교 연산자 : =, !=, >, <, >=, <=, BETWEEN A AND B (A이상 B이하)
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음
--> TIP! 같은 테이블이 왼쪽, 오른쪽에 각각 있다고 생각하고 조인하면 쉽다
...
FROM EMPLOYEE
NATURAL JOIN JOB;
--> EMPLYEE, JOB 두 테이블에서 같은 이름의 컬럼(JOB_CODE)을 기준으로 조인이 수행됨


-- NATURAL JOIN 실패 --> CROSS JOIN 결과가 조회됨
-- EMPLOYEE, DEPARTMENT 자연 조인
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
NATURAL JOIN DEPARTMENT;
--> 실패 원인 : 두 테이블에 같은 이름의 컬럼이 없어서 자연 조인 실패
-- 해결 방법 : 이너조인으로 바꿔준다
...
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) -- DEPARTMENT 테이블 JOIN
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE); -- LOCATION 테이블 JOIN

-- (주의사항!!!) 다중 조인은 반드시 순서대로 작성되어야 한다!!!
SELECT EMP_ID, EMP_NAME, JOB_NAME, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
-- 오라클 전용 : AND 연산을 이용해서 JOIN을 여러 번 진행할 수 있다.
SELECT EMP_ID, EMP_NAME, JOB_NAME, DEPT_TITLE, LOCAL_NAME
...
--연봉에 보너스포인트를 적용하시오.
SELECT DISTINCT EMP_NAME, JOB_NAME, SALARY, (SALARY+(SALARY*NVL(BONUS, 0)))*12 "연봉"
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
JOIN SAL_GRADE USING (SAL_LEVEL)
